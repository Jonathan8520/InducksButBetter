...
function App() {
  ...
  // Les réglages ne sont plus une destination mais une parenthèse : ils se superposent à
  // la page au lieu de la remplacer, ce qui évite de perdre son contexte de recherche.
  ...
  useEffect(() => {
    const handleUrlRouting = () => {
      ...
      if (rootPart === "settings") {
        // Les réglages sont une surcouche : l'URL les ouvre sans changer d'onglet.
  ...
  return (
    <TooltipProvider>
      ...
      <div id="main-content" className="min-h-screen overflow-x-hidden bg-background text-foreground">
        <div className="flex flex-col min-h-screen">
          {/* Main Header */}
        <header className="px-4 lg:px-12 py-4 border-b border-border-subtle bg-background">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div className="flex flex-col gap-0.5">
              <div className="flex items-center gap-2">
                <svg className="text-foreground dark:text-white" xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
            ...
            <div className="flex flex-row items-center gap-2">
              <Button
                ...
                className={cn(
                  ...
                  settingsOpen && "border-border-subtle bg-surface-2 text-primary"
                )}
                title={String(t("settings.title", { defaultValue: "Paramètres" }))}
              >
        ...
        <Tabs value={activeTab} onValueChange={handleTabChange} className="flex-1 flex flex-col">
            {/* Seule la barre d'onglets est collante : l'en-tête (titre et sous-titre) peut
                défiler hors de l'écran, la navigation doit rester atteignable. */}
            <div className="px-4 lg:px-12 shrink-0 flex w-full bg-surface border-b border-border-subtle py-2 sticky top-0 z-30">
              <TabsList className="bg-surface-2/90 gap-1 h-11 p-1 rounded-xl border border-border-subtle inline-flex items-center max-w-full overflow-x-auto overflow-y-hidden">
                <TabsTrigger
                  value="stories"
                  className="data-[state=active]:bg-surface data-[state=active]:text-foreground data-[state=active]:shadow-sm rounded-lg px-4 py-1.5 flex gap-2 items-center justify-center text-sm font-medium transition-all"
                >
                ...
                <TabsTrigger
                  value="publications"
                  className="rounded-lg px-4 py-1.5 flex gap-2 items-center justify-center text-sm font-medium opacity-70 hover:opacity-100 data-[state=active]:opacity-100 data-[state=active]:bg-surface data-[state=active]:text-foreground data-[state=active]:shadow-sm transition-all"
                >
          ...
          <div className="flex-1 relative">
            <TabsContent value="stories" className="m-0 p-0 border-none outline-none">
            ...
            <TabsContent value="sql" className="m-0 p-0 border-none outline-none bg-surface">
            ...
            <TabsContent value="publications" className="m-0 p-0 border-none outline-none">
              <Suspense fallback={<TabFallback />}>
            ...
            <TabsContent value="authors" className="m-0 p-0 border-none outline-none">
              <Suspense fallback={<TabFallback />}>
                <AuthorsSearch
            ...
            <TabsContent value="characters" className="m-0 p-0 border-none outline-none">
              <Suspense fallback={<TabFallback />}>
                <CharactersSearch
